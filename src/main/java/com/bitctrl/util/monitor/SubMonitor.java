 ...
 * <li>SubMonitor has a setWorkRemining method that allows the remaining space
 ...
 * <li>Use <code>SubMonitor.newChild(...)</code> whenever you need to call
 * another method that accepts an IProgressMonitor.</li>
 * </ul>
 * <p>
 ...
 * 
 * <p>
 * When writing JavaDoc for a method that accepts an IProgressMonitor, you
 * should assume the following default behavior unless the method's JavaDoc says
 * otherwise:
 * </p>
 * <ul>
 ...
 * <ul>
 * <li>It WILL call beginTask on the IProgressMonitor.</li>
 * <li>It WILL accept a null argument, indicating that no progress should be
 * reported and the operation cannot be cancelled.</li>
 * <li>It WILL NOT call done on the IProgressMonitor, leaving this
 * responsibility up to the caller.</li>
 * </ul>
 * <p>
 * If you wish to follow these conventions, you may copy and paste the following
 * text into your method's JavaDoc:
 * </p>
 * 
 * <pre>
 ...
 * <code>convert</code> or <code>progress.newChild()</code>. In this example,
 * there is no guarantee that <code>monitor</code> is an instance of
 ...
 * // user. It is the caller's responsibility
 * // to call done() on the given monitor. Accepts &lt;code&gt;null&lt;/code&gt;, indicating
 ...
 * 
 * <p>
 * <b>Example: Default usage</b>
 * </p>
 * 
 * <p>
 * You will often need to implement a method that does not explicitly stipulate
 * that calling done() is the responsibility of the caller. In this case, you
 * should use the following pattern:
 * </p>
 * 
 * <pre>
 * // param monitor the progress monitor to use for reporting progress to the
 * // user, or &lt;code&gt;null&lt;/code&gt; indicating
 ...
 * 	}
 * 
 * 	// Don't report any work, but ensure that we have 50 ticks remaining on
 ...
 * 	doSomeWork(progress.newChild(50));
 * } else {
 * 	// Bad: Causes the progress monitor to appear to start at 50%, wasting
 * 	// half of the
 ...
 * 	SubMonitor progress = SubMonitor.convert(monitor, 100);
 * 
 * 	// Create a new progress monitor that uses 70% of the total progress and
 * 	// will allocate one tick
 ...
 * 		// use 0.01% of the space remaining in the monitor to process the
 * 		// next node.
 ...
 */
public final class SubMonitor extends AbstractMonitor {
	 ...
	 * number of ticks that may be passed into worked(int) or newChild(int).
	 */
	...
	public SubMonitor setWorkRemaining(int workRemaining) {
		...
		if (totalForChildren > 0 && totalParent > usedForParent) {
			// Note: We want the following value to remain invariant after this
			// method returns
			final double remainForParent = totalParent * (1.0d - usedForChildren / totalForChildren);
	...
	private int consume(final double ticks) {
		...
		final int parentPosition = (int) (totalParent * usedForChildren / totalForChildren);
		final int delta = parentPosition - usedForParent;

		usedForParent = parentPosition;

		notifyMonitorListeners(usedForChildren);
	 ...
	 */
	private void internalWorked(final double work) {
		cleanupActiveChild();

		final int delta = consume(work > 0.0d ? work : 0.0d);
		if (delta != 0) {
			root.worked(delta);
		}
	}
	 ...
	 *         SubMonitor
	 */
	public SubMonitor newChild(final int totalWork) {
	 ...
	 *      ////////////////////////////////////////////////////////////////////////////
	 *      // Example 3: Demonstrates a common anti-pattern
	 *      void wrongMethod(IMonitor parent) {
	 *          SubMonitor progress = SubMonitor.convert(parent, 100);
	 *          
	 *          // WRONG WAY: Won't have the intended effect, as only one of these progress
	 *          // monitors may be active at a time and the other will report no progress.
	 ...
	 *         SubMonitor
	 */
	public SubMonitor newChild(final int totalWork, final int suppressFlags) {
		...
		// Compute the flags for the child. We want the net effect to be as
		// though the child is delegating to its parent, even though it is
		// actually talking directly to the root. This means that we need to
		// compute the flags such that - even if a label isn't suppressed by the
		// child - if that same label would have been suppressed when the child
		// delegated to its parent, the child must explicitly suppress the
		// label.
		int childFlags = SUPPRESS_NONE;

		if ((flags & SUPPRESS_SETTASKNAME) != 0) {
			// If the parent was ignoring labels passed to setTaskName, then the
			// child will ignore labels passed to either beginTask or
			// setTaskName - since both delegate to setTaskName on the parent
			childFlags |= SUPPRESS_SETTASKNAME | SUPPRESS_BEGINTASK;
		}

		if ((flags & SUPPRESS_SUBTASK) != 0) {
			// If the parent was suppressing labels passed to subTask, so will
			// the child.
			childFlags |= SUPPRESS_SUBTASK;
		}

		// Note: the SUPPRESS_BEGINTASK flag does not affect the child since
		// there is no method on the child that would delegate to beginTask on
		// the parent.
		childFlags |= suppressFlags;

		final SubMonitor result = new SubMonitor(root, consume(totalWorkDouble), (int) totalWorkDouble, childFlags);
	...
	private void cleanupActiveChild() {
		...
		final IMonitor child = lastSubMonitor;
		lastSubMonitor = null;
		child.done();
	}
