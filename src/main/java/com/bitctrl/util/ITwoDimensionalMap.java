...
public interface ITwoDimensionalMap<R, C, D> {
	...
	/**
	 * Returns all known row keys. This is backed by the map so that changes to the
	 * map get reflected.
	 * 
	 ...
	 * returned collection is unmodifiable and doesn't reflect changes to the map.
	 * The values are null for unset columns.
	 * 
	 * @param row the row key
	 * @return all known values for the given row.
	 */
	Collection<D> getRow(final R row);

	/**
	 * Return all known column keys. This is backed by the map so that changes to
	...
	/**
	 * Returns all currently known column entry values for the given column key. The
	 * returned collection is unmodifiable and doesn't reflect changes to the map.
	 * The values are null for unset rows.
	 * 
	 * @param column the column key
	 * @return all known values for the given column.
	 */
	Collection<D> getColumn(final C column);
	 ...
	 */
	D put(final R row, final C column, final D value);
